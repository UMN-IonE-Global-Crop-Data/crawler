...
def unclick_crop(driver, index, page):
    ...
    # /html/body/div[1]/div[1]/div/div[3]/div[2]/div[1]/div/div[4]/div[2]/div/div[2]/div[2]/div[3]/div[4]/table/tbody[1]/tr[1]/td/div

    # scroll to the end the window
    # if index == 0:
...
def select_crop_and_unit(driver, index, page):
    crop_table_xpath = "/html/body/div[1]/div[1]/div/div[3]/div[2]/div[1]/div/div[4]/div[2]/div/div[2]/div[2]/div[3]/div[4]/table/tbody[1]/tr"
    # crop_table_xpath = "//div[@class='z-listitem']/div[4]/div[2]/table/tbody[1]/tr"

    # 返回之后需要取消掉上一次的选中的crop
    unclick_crop(driver, index, page)

    # Re-find the elements within the loop to handle stale element reference
    crop_table = driver.find_elements(By.XPATH, crop_table_xpath)
    ...
    # only need to click at the first time
    # if page == 1 and index == 0: # for test only
...
def select_field_and_get_crop_table(driver, xpath, page):
    crop_table_xpath = "/html/body/div[1]/div[1]/div/div[3]/div[2]/div[1]/div/div[4]/div[2]/div/div[2]/div[2]/div[3]/div[4]/table/tbody[1]/tr"
    if page == 1:
...
def select_all_years(driver):
    all_year_button = driver.find_element(By.XPATH, "/html/body/div[1]/div[1]/div/div[3]/div[2]/div[2]/div/div[1]/div[2]/table/tbody/tr/td/table/tbody/tr/td[1]/div/div[1]/table/tbody/tr/th[1]/div/span").click()
    time.sleep(0.5)
...
def select_all_regions(driver, level):
    ...
    try:
        option = driver.find_element(By.XPATH, f'''{region_dropdown_xpath}//option[text()="{level}"]''').click()
    ...
    all_region_button = WebDriverWait(driver, 15).until(
...
def generate_report(driver):
    time.sleep(0.5)
    create_report_button = driver.find_element(By.XPATH, "/html/body/div[1]/div[1]/div/div[3]/div[2]/div[3]/div/div[2]/button[2]").click()
    time.sleep(0.5)
    try:
...
def rename_and_move_file(level):
    ...
    for file in os.listdir(src_dir):
        if file.endswith(".xls"):
            ...
            crop_name = crop_name_text[start_idx:end_idx]
            
            print(f"rename file, orginal text {text[1]}, get crop name: {crop_name}")

            file_name = f"{data_type}_{crop_name}.xls".replace("/", "_")
            
            target_dir = os.path.join("raw", level, crop_name)
...
def merge_all_files(target_dir):
    for item in os.listdir(target_dir):
        item_path = os.path.join(target_dir,item)

        if os.path.isfile(item_path):
            generate_final_file(target_dir)
            return

        elif os.path.isdir(item_path):
            merge_all_files(item_path)
...
def generate_final_file(target_dir):
    ...
    # print(target_dir)
    # processed_path = os.path.join(".", "processed", target_dir)
    # if not os.path.exists(processed_path):
    #     os.makedirs(processed_path)

    res.to_csv(f".\\processed\\NUTS3 (Province Level)\\{crop_name}.csv", index = False, encoding="utf-8-sig")


def merge(level):
    path = os.path.join(os.getcwd(), "raw", level)

    for item in os.listdir(path):
        merge_all_files(os.path.join(path,item))
